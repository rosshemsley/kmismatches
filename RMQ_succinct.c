...
static inline DTidx log2fast(DTidx v) {
...
void RMQ_succinct(DT* a, DTidx n) {
	...
	if (nb<sprimeprime/(2*sprime)) { fprintf(stderr, "Array too small...exit\n"); exit(-1); }

	// Type-calculation for the microblocks and pre-computation of in-microblock-queries:
	type = malloc(sizeof(DTsucc2) * nmb);          // new DTsucc2[nmb];
#ifdef MEM_COUNT
	unsigned long mem = sizeof(DTsucc2)*nmb;
#endif
	Prec = malloc (sizeof(DTsucc*)*Catalan[s][s]); // new DTsucc*[Catalan[s][s]];
	for (DTidx i = 0; i < Catalan[s][s]; i++) {
		Prec[i] = malloc(sizeof(DTsucc)*s);         // new DTsucc[s];
#ifdef MEM_COUNT
		mem += sizeof(DTsucc)*s;
#endif
	...
	DT* rp = malloc(sizeof(DT) * (s+1)  );  // new DT[s+1];   // rp: rightmost path in Cart. tree
	...
	DTidx* gstack = malloc(sizeof(DTidx) * s); // new DTidx[s];
	DTidx gstacksize;
	...
	free(rp); //delete[] rp;
	free(gstack); //delete[] gstack;

	// space for out-of-block- and out-of-superblock-queries:
	M_depth = (DTidx) floor(log2(((double) sprimeprime / (double) sprime)));
	M    = malloc(sizeof(DTsucc*) * M_depth); // new DTsucc*[M_depth];
	M[0] = malloc(sizeof(DTsucc) * nb); // new DTsucc[nb];
#ifdef MEM_COUNT
	mem += sizeof(DTsucc)*nb;
#endif
	Mprime_depth = (DTidx) floor(log2(nsb)) + 1;
	Mprime    = malloc(sizeof(DTidx*) * Mprime_depth); // new DTidx*[Mprime_depth];
	Mprime[0] = malloc(sizeof(DTidx)* nsb);  // new DTidx[nsb];
#ifdef MEM_COUNT
	...
	for (DTidx j = 1; j < M_depth; j++) {
		M[j] = malloc(sizeof(DTsucc) * nb); // new DTsucc[nb];
#ifdef MEM_COUNT
	    mem += sizeof(DTsucc)*nb;
#endif
	...
	for (DTidx j = 1; j < Mprime_depth; j++) {
		Mprime[j] = malloc(sizeof(DTidx) * nsb); // new DTidx[nsb];
#ifdef MEM_COUNT
        mem += sizeof(DTidx)*nsb;
#endif
...
void FreeRMQ_succinct() {
	free (type); // delete[] type;
	for (DTidx i = 0; i < Catalan[s][s]; i++) free(Prec[i]); //delete[] Prec[i];
	free(Prec); // delete[] Prec;
	for (DTidx i = 0; i < M_depth; i++) free(M[i]); // delete[] M[i];
	free(M); //delete[] M;
	for (DTidx i = 0; i < Mprime_depth; i++) free(Mprime[i]); // delete[] Mprime[i];
	free(Mprime); //delete[] Mprime;
}
