...
int start_p_triple(char t, const char *pattern, const int *SA, const int *LCP, int n)
{
...
int extend_p_triple(char t, const char * pattern, const int *SA, const int *LCP, int n)
{
...
void construct_p_representation(const char *text, const char *pattern, const int *SA, const int *LCP, int n)
{
...
void constructSA(const char *text, const char *pattern, int n, int m)
{
                                ...
                                int  *matches  )
{
  ...
  if(!fftw_import_system_wisdom()){
   ...
   int FREQ_CHAR_THRESHOLD =  0.05* sqrt(m * log(m)/log(2) );
		                               ...
		                               unsigned int  flags    )
{
  ...
  // int MATCH_THRESHOLD     = 2 * sqrt(k);
   ...
   if (1) //count_frequent_symbols(FREQ_CHAR_THRESHOLD, frequency_table, n) > MATCH_THRESHOLD)
   {
      // This will become a look up for each frequent character.
      int pattern_lookup[FREQ_CHAR_THRESHOLD];
      ...
      else
   {
      ...
      // ---------------------------------------------------------------------//   
      // Perform mismatching by kangarooing //
      // ---------------------------------------------------------------------//   
      
      
      constructSA(text, pattern, n, m);
...
int loadData(char ** data, const char *filename)
{
   FILE *fp;
   long n;
...
int main(int argc, char **argv)
{
   ...
   int * A = malloc(sizeof(int) * n-m+1);
   
  abrahamson_kosaraju(t,p,n,m, A);
